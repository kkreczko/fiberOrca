...
int main(int argc, char **argv) {
  ...
  if (setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE, opt, strlen(opt) + 1) ==
      -1) {
    close(sock);
  ...
  struct sock_filter BPF_code[] = {
      ...
      {0x30, 0, 0, 0x00000017}, {0x15, 6, 7, 0x00000006},
      {0x15, 0, 6, 0x000086dd}, {0x30, 0, 0, 0x00000014},
      {0x15, 3, 0, 0x00000006}, {0x15, 0, 3, 0x0000002c},
      {0x30, 0, 0, 0x00000036}, {0x15, 0, 1, 0x00000006},
  ...
  // We don't need that as the program will work only with frontend approach in
  // mind Verbose mode I think should go as it's useless
  while (1) { // pcap_loop
    printf("-----------\n");
    ...
    if (*iphead == 0x45) {
      printf("Source host %d.%d.%d.%d\n", iphead[12], iphead[13], iphead[14],
             iphead[15]);
      printf("Dest host %d.%d.%d.%d\n", iphead[16], iphead[17], iphead[18],
             iphead[19]);
